...
class ResBlock(nn.Module):
    def __init__(self, n_feats, kernel_size=3,bias=True, bn=False, act=nn.ReLU(True), res_scale=1):
        ...
        m = []
        for i in range(2):
            m.append(nn.Conv2d(n_feats, n_feats, kernel_size,1,1))
            m.append(nn.Conv2d(n_feats, n_feats, kernel_size, padding=(kernel_size//2), bias=bias, groups=n_feats))
            if bn:
...
class LocalKernelEstimator(nn.Module):
    def __init__(self, dim, filer_h, filter_w, dynamic = False) -> None:
        super(LocalKernelEstimator,self).__init__()
        # self.process = LocalKFourierBlock(dim, filer_h, filter_w, dynamic)
        self.process = nn.Sequential(*[ResBlock(dim) for i in range(2)])
        self.GenWeight = nn.Sequential(
            nn.Conv2d(dim,dim,3,1,1),
            nn.LeakyReLU(0.2),
            nn.Conv2d(dim,dim*2,3,1,1)
    ...
    def __init__(self,dim=28,stage=2,n_s=[1,1,1],num_blocks=[2,2,2],image_size=256,Multiscale=False,type='LocalTrans',input_fusion=True):
        ...
        if self.input_fusion:
            self.fution = nn.Sequential(
                ...
                nn.Conv2d(28, 28,3,1,1,groups=dim)
                )
        ...
        # Decoder
        self.decoder_layer = TransBlock(self.dim,filter_h,filter_w,1,2,type)
        ...
        self.ffution1 = nn.Conv2d(self.dim+self.dim*2+self.dim*4, self.dim, 1, 1, 0, bias=False)
        self.ffution2 = nn.Conv2d(self.dim, self.dim,3,1,1,groups=dim)
